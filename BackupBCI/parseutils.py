...
def extract_from_part(parsed, stop_at_punctuation=True):
    ...
    for item in parsed.tokens:
        print(str(item.ttype) + " --> " + str(item.value.upper()))
        if tbl_prefix_seen:
...
if __name__ == "__main__":
    sql = '''SELECT AP.Party_Id
		    FROM ARMVIEWS.RMJ_PROD_PRODUCT PP 
		    INNER JOIN ARMVIEWS.RMR_CLT_ACC_PROD_PROD_A APP
		    ON PP.PRODUCT_ID=APP.PRODUCT_ID 
		    INNER JOIN EDW_VW.ACCOUNT_PARTY AP
		    ON APP.ACCOUNT_NUM = AP.ACCOUNT_NUM
		     LEFT OUTER JOIN EDW_VW.BCI_LEGAL_DOC B
		    ON  B.ACCOUNT_NUM = AP.ACCOUNT_NUM 
		     AND  B.LEDGER_STATUS_IND  IN ('VCD','CAS')
		    LEFT OUTER JOIN EDW_VW.ACCT_ACCT_RELATIONSHIP C
		     ON  C.RELATED_ACCOUNT_NUM = AP.ACCOUNT_NUM 
		     AND  C.ACCT_RELATIONSHIP_TYPE_CD = 3
		    WHERE  PP.CATEGORY_CD IN ('Hipotecario') 
			     AND ACCOUNT_PARTY_ROLE_CD      = 7 /*TITULAR */  
			     AND  ACCOUNT_PARTY_END_DT IS NULL
		    GROUP BY 1,2,3,4
		    QUALIFY ROW_NUMBER() OVER (PARTITION BY AP.ACCOUNT_NUM ORDER BY AP.ACCOUNT_PARTY_START_DT  DESC) = 1'''  
